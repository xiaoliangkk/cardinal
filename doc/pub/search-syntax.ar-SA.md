...
- تتم المطابقة حسب **مكوّنات المسار**:
  - الكلمات والعبارات والبدائل التي لا تحتوي على `/` تطابق اسم الملف أو المجلد نفسه.
  - الرموز المفصولة بـ `/` تطابق سلسلة متجاورة من مكوّنات المسار وتعيد العنصر الذي يطابق آخر مقطع.
  - عوامل المنطق البولياني تجمع مجموعات النتائج للعنصر المفهرس نفسه؛ يعني `foo bar` أن عنصرًا واحدًا يجب أن يطابق الرمزين معًا، لا أن أسلافه يمكن أن يطابقوا رمزًا واسمه الأساسي يطابق الرمز الآخر.
- تتحكم واجهة المستخدم بحساسية حالة الأحرف:
  - تستخدم مطابقة الاسم/المسار هذا المفتاح مباشرة.
  - يمرر `content:` الإعداد نفسه إلى Spotlight.
...
- مثل الرموز العادية، رموز البدائل التي لا تحتوي على `/` تطابق مكوّنات المسار. سلسلة بدائل مفصولة بشرطات مائلة مثل `src/**/Cargo.toml` تعيد عناصر `Cargo.toml` المطابقة، بينما `src/**` يعيد العناصر التابعة تحت مجلدات `src` المطابقة.
...
يبحث `content:` في فهرس محتوى macOS Spotlight عن **جزء نصي بسيط**:

- لا يوجد regex داخل `content:`؛ تُرسل القيمة إلى Spotlight كمحتوى نصي.
- حساسية حالة الأحرف تتبع مفتاح UI عبر معدّل استعلام Spotlight.
- تُسمح الكلمات القصيرة جدًا، لكن `""` (الفارغ) مرفوض.
- تُرفض القيم التي تحتوي على `*` أو `'` أو `\` لأن هذه الأحرف تؤثر في صياغة استعلام Spotlight.
- تعتمد النتائج على فهرسة Spotlight وأنواع الملفات التي يستطيع Spotlight استخراج النص منها.
...
مطابقة المحتوى لا تقرأ أجسام الملفات مباشرة؛ بل تستخدم Spotlight فقط.
