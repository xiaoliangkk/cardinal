...
- 大文字/小文字の区別は UI のトグルで制御されます:
  - 名前/パスのマッチングはこのトグルを直接使用します。
  - `content:` は同じ設定を Spotlight に渡します。
...
`content:` は macOS Spotlight の内容インデックスで **単純な部分文字列** を検索します:

- `content:` 内に regex はありません。値はテキスト内容として Spotlight に渡されます。
- 大文字/小文字の扱いは Spotlight のクエリ修飾子を通じて UI トグルに従います。
- 非常に短い検索語は許可されますが、`""`（空）は拒否されます。
- `*`、`'`、`\` を含む値は、Spotlight クエリ構文に影響するため拒否されます。
- 結果は Spotlight のインデックス状態と、Spotlight がテキストを抽出できるファイルタイプに依存します。
...
内容の一致ではファイル本文を直接読みません。Spotlight のみを使用します。
