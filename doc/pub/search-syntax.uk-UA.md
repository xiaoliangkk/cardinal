...
- Зіставлення орієнтоване на **компоненти шляху**:
  - Слова, фрази й підстановки без `/` збігаються з власною назвою файла або папки.
  - Токени, розділені `/`, збігаються з неперервним ланцюжком компонентів шляху й повертають елемент, що збігся з останнім сегментом.
  - Булеві оператори комбінують набори результатів для одного й того самого індексованого елемента; `foo bar` означає, що один елемент має збігтися з обома токенами, а не що його предки можуть задовольняти один токен, а базова назва інший.
- Чутливість до регістру керується перемикачем в UI:
  - Зіставлення імені/шляху використовує цей перемикач напряму.
  - `content:` передає те саме налаштування Spotlight.
...
- Як і прості токени, підстановки без `/` зіставляються з компонентами шляху. Ланцюжок підстановок зі слешами, наприклад `src/**/Cargo.toml`, повертає збіглі елементи `Cargo.toml`, а `src/**` повертає нащадків під збіглими папками `src`.
...
`content:` шукає **простий підрядок** в індексі вмісту macOS Spotlight:

- У `content:` немає regex; значення передається Spotlight як текстовий вміст.
- Чутливість до регістру слідує перемикачу UI через модифікатор запиту Spotlight.
- Дуже короткі “голки” дозволені, але `""` (порожній) відхиляється.
- Значення з `*`, `'` або `\` відхиляються, бо ці символи впливають на синтаксис запитів Spotlight.
- Результати залежать від індексації Spotlight і типів файлів, з яких Spotlight може витягти текст.
...
Збіг вмісту не читає тіла файлів напряму; використовується лише Spotlight.
