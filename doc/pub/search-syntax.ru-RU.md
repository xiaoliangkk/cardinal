...
- Чувствительность к регистру управляется переключателем в UI:
  - Сопоставление имени/пути использует этот переключатель напрямую.
  - `content:` передает ту же настройку Spotlight.
...
`content:` ищет **простую подстроку** в индексе содержимого macOS Spotlight:

- В `content:` нет regex; значение передается Spotlight как текстовое содержимое.
- Чувствительность к регистру следует переключателю UI через модификатор запроса Spotlight.
- Очень короткие подстроки разрешены, но `""` (пустая) отклоняется.
- Значения с `*`, `'` или `\` отклоняются, потому что эти символы влияют на синтаксис запросов Spotlight.
- Результаты зависят от индексации Spotlight и типов файлов, из которых Spotlight умеет извлекать текст.
...
Сопоставление содержимого не читает тела файлов напрямую; используется только Spotlight.
