...
- 대/소문자 구분은 UI 토글로 제어됩니다:
  - 이름/경로 매칭은 이 토글을 직접 사용합니다.
  - `content:`는 같은 설정을 Spotlight에 전달합니다.
...
`content:`는 macOS Spotlight 콘텐츠 인덱스에서 **일반 문자열 부분**을 검색합니다:

- `content:` 안에는 regex가 없습니다. 값은 텍스트 콘텐츠로 Spotlight에 전달됩니다.
- 대/소문자 구분은 Spotlight 쿼리 수정자를 통해 UI 토글을 따릅니다.
- 매우 짧은 문자열은 허용되지만 `""`(빈 문자열)은 거부됩니다.
- `*`, `'` 또는 `\`가 포함된 값은 Spotlight 쿼리 문법에 영향을 주므로 거부됩니다.
- 결과는 Spotlight 인덱싱 상태와 Spotlight가 텍스트를 추출할 수 있는 파일 형식에 따라 달라집니다.
...
콘텐츠 매칭은 파일 본문을 직접 읽지 않으며 Spotlight만 사용합니다.
