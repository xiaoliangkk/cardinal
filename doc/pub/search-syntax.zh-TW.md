...
`content:` 使用 macOS Spotlight 內容索引搜尋 **一般子字串**：

- `content:` 中不支援 regex；輸入會作為文字內容查詢交給 Spotlight。
- 大小寫敏感透過 Spotlight 查詢修飾符遵循 UI 開關。
- 允許非常短的搜尋詞，但 `""`（空）會被拒絕。
- 包含 `*`、`'` 或 `\` 的值會被拒絕，因為這些字元會影響 Spotlight 查詢語法。
- 結果取決於 Spotlight 是否已經索引，以及 Spotlight 能否從對應檔案類型中提取文字。
